...
INTEGER,PARAMETER::MV=5
CHARACTER(LEN=100)::VNAME(MV)
...
namelist /para/INDIR,ODIR,Y,NV,IM,JM,NM
...
var2d=0.0
WRITE(YYYY,'(I4)') Y
INFLE="J-OFURO3_"//TRIM(VNAME(I))//"_DAILY_123_21_"//YYYY//".nc"
IN=TRIM(INDIR) // '/' // TRIM(VNAME(I)) // '/' //TRIM(INFLE)

CALL READ_NC(VNAME(I), IN, IM,JM,NM,var2d)
...
OFLE="J-OFURO3_"//TRIM(VNAME(I))//"_DAILY_123_21_"//YYYY//"_CHK.nc"
OUT=TRIM(ODIR) // '/' // TRIM(VNAME(I)) // '/' //TRIM(INFLE)
...
OUT=TRIM(ODIR) // '/' // TRIM(VNAME(I)) // '/' //TRIM(OFLE)

CALL WRITE_NC(VNAME(I), OUT, IM, JM, NM, var2d)
