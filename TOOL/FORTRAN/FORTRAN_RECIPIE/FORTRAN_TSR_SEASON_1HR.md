...
Fri, 17 Jul 2020 18:33:33 +0900
calypso.bosai.go.jp
/work05/manda/20.AGO_WAN/MIE_MON/TSR_SEASON_1HR

```
srcdump.sh TSR_SEASON_1HR.sh TSR_SEASON_1HR.f90 PL_TSR_SEASON_1HR.sh
```
...
INFLE=AGO_WANOU.txt
ODIR=.
OFLE=$(basename $INFLE .txt)_SEAS_1HR_TEST.txt
BFLE=$(basename $INFLE .txt)_SEAS_1HR_TEST.bin
CTL=$(basename $INFLE .txt)_SEAS_1HR_TEST.CTL
...
if(x1(n) /= rmiss)then
A1(JD,IHH)=A1(JD,IHH)+x1(n)
N1(JD,IHH)=N1(JD,IHH)+1.0
endif

if(x2(n) /= rmiss)then
A2(JD,IHH)=A2(JD,IHH)+x2(n)
N2(JD,IHH)=N2(JD,IHH)+1.0
endif

if(x3(n) /= rmiss)then
A3(JD,IHH)=A3(JD,IHH)+x3(n)
N3(JD,IHH)=N3(JD,IHH)+1.0
endif

if(x4(n) /= rmiss)then
A4(JD,IHH)=A4(JD,IHH)+x4(n)
...
do JD=1,366
do IHH=0,23
if(N1(JD,IHH)>3.0)then
A1(JD,IHH)=A1(JD,IHH)/N1(JD,IHH)
else
A1(JD,IHH)=rmiss
endif

if(N2(JD,IHH)>3.0)then
...
yyyy(n),mm(n),dd(n),hh(n),XC1(n),XC2(nt),XC3(nt),XC4(nt)
end do !i
close(21)
...
CTL=AGO_WANOU_SEAS_1HR_TEST.CTL

YYYY=2020


FIG=$(basename $CTL .CTL)_$YYYY.png
