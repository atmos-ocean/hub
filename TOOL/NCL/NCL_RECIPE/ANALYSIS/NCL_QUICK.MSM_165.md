...
NCAR Command languageを使用した例です。


R: 3時間雨量 [mm]（表記された時刻の前3時間の積算雨量）
...
注意: decameter (dam)はメートルを10で割った値。



[2017年  2月 21日 火曜日 13:54:24 JST]

[~/2017.Heavy.Rain/MSM.Quick.Look]

[am@aofd165]

```shell
$ quick.msm.run.sh 20160829
$ srcdump.sh quick.msm.run.sh quick.msm.ncl runncl.sh 0.README.txt
```
...
/work1/am/2017.Heavy.Rain/MSM.Quick.Look

Tue Feb 21 14:11:43 JST 2017
...
yyyy0=${yyyymmdd0:0:4}
  mm0=${yyyymmdd0:4:2}
  dd0=${yyyymmdd0:6:2}
indir=/mnt/raid112MANDA/KakenA.H28/MSM

outdir="Fig"

mkdir -vp $outdir
indirp0=$indir/${yyyy0}/MSM-P
indirs0=$indir/${yyyy0}/MSM-S
yyyymmdd1=$(date -d"$yyyymmdd0"-1days '+%Y%m%d')
yyyy1=${yyyymmdd1:0:4}
  mm1=${yyyymmdd1:4:2}
  dd1=${yyyymmdd1:6:2}
indirp1=$indir/${yyyy1}/MSM-P
indirs1=$indir/${yyyy1}/MSM-S
...
; http://www.jma.go.jp/jma/kishou/know/faq/faq1.html
  ...
  res1=opts_cs
  res1@vpXF = 0.1
  res1@gsnLeftString   = "R [mm/"+ dthr +"hr]"
...
; Water vapor mixing ratio
  ...
  tc=tp-273.15
  es=611.0/100.*10^((7.5*tc)/(237.3+tc)) ;[hPa] saturated water vapor
;                                         pressure by Tetens (1930)'s formulae
